...
// Section 5 historical-logs cascade: the master's HTTP API falls back to
// this when the sensor_id+sensor_type it was asked about isn't in the
// master's own DB. Unlike handle_incoming_request() above (single latest
// value), this returns every reading for the given sensor_id+sensor_type
// on the given calendar date, read straight from this slave's own SQLite
// DB (never from Memcached -- a date-scoped history query isn't a single
// cacheable key/value pair the way "latest value" is).
void handle_incoming_logs_request(const std::string& payload) {
    std::string sensor_id = extract_json_field(payload, "id");
    std::string sensor_type = extract_json_field(payload, "sensor_type");
    std::string date = extract_json_field(payload, "date");
    std::string corr_id = extract_json_field(payload, "correlation_id");
    ...
    if (sqlite3_open_v2(g_db_path.c_str(), &db, SQLITE_OPEN_READONLY, nullptr) != SQLITE_OK) {
        ...
        pubmsg.payloadlen = (int)response.length();
        pubmsg.qos = 1;
        MQTTClient_publishMessage(g_mqtt_client, "cluster/slave/logs_response", &pubmsg, nullptr);
        return;
    }
    ...
    bool sensor_exists = false;
    {
        ...
        if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK) {
            ...
            if (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                sensor_name = v ? reinterpret_cast<const char*>(v) : "";
                sensor_exists = true;
            }
            sqlite3_finalize(stmt);
        }
    ...
    // "values" is ';'-separated "time|value" pairs -- same manual-encoding
    // style the project already uses (REC_SEP-encoded cache records) --
    // rather than a nested JSON array, so the master can parse it with a
    // plain split instead of needing a JSON array parser.
    std::string values_encoded;
    if (sensor_exists) {
        const char* sql =
            "SELECT time(recorded_at), value FROM sensor_readings "
            "WHERE sensor_id = ? AND date(recorded_at) = ? ORDER BY recorded_at ASC;";
        sqlite3_stmt* stmt = nullptr;
        if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK) {
            sqlite3_bind_text(stmt, 1, sensor_id.c_str(), -1, SQLITE_STATIC);
            ...
            while (sqlite3_step(stmt) == SQLITE_ROW) {
                ...
                if (!first) values_encoded += ";";
                values_encoded += (t ? reinterpret_cast<const char*>(t) : "");
                values_encoded += "|";
                values_encoded += (v ? reinterpret_cast<const char*>(v) : "");
                first = false;
            }
            sqlite3_finalize(stmt);
        }
...
int main() {
    ...
    MQTTClient_subscribe(g_mqtt_client, "cluster/slave/logs_request", 1);
    ...
    while (g_running) {
        ...
        if (rc == MQTTCLIENT_SUCCESS && message != nullptr) {
            ...
            if (topic == "cluster/slave/logs_request") {
                handle_incoming_logs_request(payload);
            } else {
