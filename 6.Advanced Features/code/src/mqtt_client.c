...
struct mqtt_client {
    ...
    char alarm_topic[CFG_MAXLEN * 2 + 32];
};
...
mqtt_client_t *mqtt_client_start(const notifier_config_t *cfg) {
    ...
    /* Deliberately prefix/id/alarm, NOT prefix/alarm/id like the two
     * above — this exact shape ("home/<student_id>/alarm") is what the
     * Step 6 spec calls for. */
    snprintf(client->alarm_topic, sizeof(client->alarm_topic),
             "%s/%s/alarm", cfg->mqtt_topic_prefix, cfg->student_id);
...
void mqtt_client_publish_alarm(mqtt_client_t *client, const notifier_config_t *cfg,
    ...
    char payload[256];
    int len = snprintf(payload, sizeof(payload),
        "{\"count\": %d, \"timestamp\": \"%s\", \"student_id\": \"%s\", \"alarm\": true}",
        count, timestamp, cfg->student_id);

    /* Not debounced at the MQTT layer — only email is rate-limited per
     * spec. A guard-mode subscriber (the "PC" watching for alarms) is
     * meant to see every triggering frame, not a throttled subset. */
    int rc = mosquitto_publish(client->mosq, NULL, client->alarm_topic,
                                len, payload, 1 /* QoS 1 */, false /* not retained */);
    if (rc != MOSQ_ERR_SUCCESS) {
        fprintf(stderr, "[mqtt] publish to %s failed: %s\n",
                client->alarm_topic, mosquitto_strerror(rc));
    }
}
