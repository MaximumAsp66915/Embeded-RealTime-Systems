...
static void set_defaults(notifier_config_t *cfg) {
    ...
    /* --- Step 6 defaults --- */
    snprintf(cfg->guard_state_path, sizeof(cfg->guard_state_path), "/dev/shm/surveillance/guard_state.json");

    snprintf(cfg->blackbox_db_path, sizeof(cfg->blackbox_db_path), "/opt/surveillance/notifier/blackbox.db");
    cfg->blackbox_max_events = 1000;

    cfg->watchdog_timeout_s = 30;
    snprintf(cfg->watchdog_service_name, sizeof(cfg->watchdog_service_name), "surveillance-imgproc");

    snprintf(cfg->thermal_control_path, sizeof(cfg->thermal_control_path), "/dev/shm/surveillance/control.json");
    cfg->thermal_trigger_c = 70.0;
    cfg->thermal_recover_c = 65.0;
    cfg->thermal_normal_fps = 15.0;
    cfg->thermal_normal_scale = 1.0;
    cfg->thermal_throttled_fps = 5.0;
    cfg->thermal_throttled_scale = 0.5;
}
...
int config_load(const char *path, notifier_config_t *out) {
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        *eq = '\0';
        ...
        else if (strcmp(key, "blackbox_max_events") == 0) out->blackbox_max_events = atoi(value);
        else if (strcmp(key, "watchdog_timeout_s") == 0)  out->watchdog_timeout_s = atoi(value);
        else if (strcmp(key, "watchdog_service_name") == 0) snprintf(out->watchdog_service_name, sizeof(out->watchdog_service_name), "%s", value);
        else if (strcmp(key, "thermal_control_path") == 0) snprintf(out->thermal_control_path, sizeof(out->thermal_control_path), "%s", value);
        else if (strcmp(key, "thermal_trigger_c") == 0)   out->thermal_trigger_c = atof(value);
        else if (strcmp(key, "thermal_recover_c") == 0)   out->thermal_recover_c = atof(value);
        else if (strcmp(key, "thermal_normal_fps") == 0)  out->thermal_normal_fps = atof(value);
        else if (strcmp(key, "thermal_normal_scale") == 0) out->thermal_normal_scale = atof(value);
        else if (strcmp(key, "thermal_throttled_fps") == 0) out->thermal_throttled_fps = atof(value);
        else if (strcmp(key, "thermal_throttled_scale") == 0) out->thermal_throttled_scale = atof(value);
