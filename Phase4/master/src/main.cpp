...
// ---------------------------------------------------------------------------
// SNMP custom OID tree configuration
//
// Base OID   : .1.3.6.1.4.1.9999
// Per sensor : <base>.<sensor_id>.1  -> name        (string)
//              <base>.<sensor_id>.2  -> description  (string)
//              <base>.<sensor_id>.3  -> last value   (string)
//
// The set of sensor IDs exposed via SNMP is configured through the
// SENSOR_IDS environment variable (comma separated). Sensor IDs are
// globally unique across the whole cluster (master + all slaves), so a
// single sensor_id is enough to resolve a sensor's full record -- no
// sensor_type needs to be known ahead of time by whoever is asking.
// ---------------------------------------------------------------------------
...
// storage in Memcached (which only stores opaque byte strings).
...
// ---------------------------------------------------------------------------
// Sensor record: everything SNMP needs (name, description material, value)
// resolved from ONE lookup by sensor_id, regardless of the sensor's type.
// ---------------------------------------------------------------------------
struct SensorRecord {
    bool found = false;
    std::string type;
    ...
    std::string unit;
    std::string value;
    std::string source; // CACHE / DB / MQTT, informational only
};
...
// Looks a sensor up in THIS node's local SQLite DB by sensor_id alone.
...
// Cache -> local DB -> MQTT cascade. Returns a fully-populated SensorRecord
// (rec.found == false if the sensor could not be resolved anywhere).
...
// ---------------------------------------------------------------------------
// SNMP pass-protocol tree helpers
// ---------------------------------------------------------------------------
...
// Finds the first leaf (id, field) strictly greater than (cur_id, cur_field)
// in ascending (id, field) order. field ranges over {1,2,3}. cur_id = -1,
// cur_field = 0 represents "before the very first leaf" (start of a walk).
...
// Builds the 3-line pass-protocol response (OID / TYPE / VALUE) for a known
// leaf. Fields 1, 2 and 3 all resolve through the SAME cache/DB/MQTT cascade
// (resolve_sensor_record), so the first field queried for a given sensor
// pays the full cascade cost and the other two come back from cache.
...
void snmp_listener_loop() {
    ...
    while (g_running) {
        ...
        if (n > 0) {
            ...
            // Expected wire format from scripts/snmp_pass.sh: "<mode>|<oid>"
            // where <mode> is "-g", "-n", or "-s".
            ...
            // mode == "-s" (SET) or anything else: agent is read-only, no response.

            if (!out_payload.empty()) {
                sendto(sockfd, out_payload.c_str(), out_payload.length(), 0, (struct sockaddr*)&clientaddr, addr_len);
            }
            // empty out_payload => intentionally send nothing; the bridge script's
            // `nc -w 1` will time out and return empty, which snmpd reports as
            // "No Such Instance" (GET) or "End of MIB view" (GETNEXT).
        }
    }
    close(sockfd);
}
