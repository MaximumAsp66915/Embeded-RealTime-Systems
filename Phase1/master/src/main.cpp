...
// Blocking payload matrix for internal HTTP client routing
...
struct ClientRequestContext {
    std::string path_query = ""; // The path + query parameters to request
    bool completed = false;
    int status_code = 0;
    std::string body = "";
};
...
static void client_cb(struct mg_connection *c, int ev, void *ev_data) {
    ...
    if (ev == MG_EV_CONNECT) {
        // The raw TCP link is open! Now we explicitly push the HTTP GET text line down the wire
        mg_printf(c,
                  "GET %s HTTP/1.0\r\n"
                  "Host: 127.0.0.1\r\n"
                  "Content-Length: 0\r\n"
        ...
        // Safely extract status code and content body using standard functions
        ...
        ctx->completed = true;
        c->is_closing = 1; // Safely shut down connection since message is received
    } 
...
std::string fetch_from_slave(const std::string& port, const std::string& type, const std::string& id) {
    ...
    conn->fn_data = &ctx; // Bind our context containing both the target query path and response slots
...
static void gateway_handler(struct mg_connection *c, int ev, void *ev_data) {
    if (ev == MG_EV_HTTP_MSG) {
        ...
        std::string lookup = check_local_db(s_type, s_id);
...
int main() {
    ...
    std::cout << "[STARTUP] Routing Configuration Matrix -> Master Port: " << g_port 
              << " | Slave 1 Port: " << g_slave1_port 
              << " | Slave 2 Port: " << g_slave2_port << std::endl;
